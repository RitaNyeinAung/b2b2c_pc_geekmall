  ...
  <div class="account">
    ...
    <div class="center">
        ...
        <div class="bottom">
          ...
          <div class="dianpu">
            ...
            <ul class="order-list">
              ...
              <li class="order-info gift-info">
                <img @click="toDetails" class="l" :src="URL + goodsData.pic_url" alt=""/>
                <p @click="toDetails" class="l">{{goodsData.title}}</p>
                <p class="l">无</p>
                <p class="l">{{goodsData.activity_price}}</p>
                ...
                <p class="l">
                  ...
                  <span style="color: #dd2727;" v-if="(goodsData.activity_price * goodsData.goods_num) >= recductionData.full && recductionData.full">
                      <!-- {{options[value].displayName}}  -->
                      满￥{{recductionData.full}}，立减￥{{recductionData.expression}}
                   </span>
                </p>
                <!-- <p v-else>
                  无
                ...
                <p v-else-if="deliveryMoney" class="r" style="width: 300px;">
                  <span v-if="goodsData.activity_price * goodsData.goods_num>=recductionData.expression">{{goodsData.activity_price * goodsData.goods_num - recductionData.expression + deliveryMoney|keep2Num }}</span>
                  <span v-else>{{goodsData.activity_price * goodsData.goods_num  + deliveryMoney|keep2Num }}</span>
                  <br>(含配送费：{{deliveryMoney|keep2Num}})
                </p>
								<p v-else-if="freight_price == 0" class="r">卖家包邮</p>
                <p v-else class="r" style="width: 300px;">
                  <span v-if="goodsData.activity_price * goodsData.goods_num>=recductionData.expression">{{goodsData.activity_price * goodsData.goods_num - recductionData.expression + freight_price|keep2Num }}</span>
                  <span v-else>{{goodsData.activity_price * goodsData.goods_num  + freight_price|keep2Num }}</span>
                  <br>(含运费费：{{freight_price|keep2Num}})
                </p>
              </li>
          
              <li class="order-info gift-info" v-if="giftData.length != 0" v-for="(id, index) in giftData" :key="index">
                
                <!-- <img @click="toDetails" class="l" :src="URL + id.pic_url" alt=""/>
                <p @click="toDetails" class="l">{{id.title}}</p> -->
                <img @click="toDetailsItem(id.id)" class="l" :src="URL + id.pic_url" alt=""/>
                <p @click="toDetailsItem(id.id)" class="l">{{id.title}}</p>
                <p class="l">无</p>
                <p class="l">{{id.activity_price}}</p>
      ...
      <div class="jiesuan">
        ...
        <p v-if="((goodsData.activity_price * goodsData.goods_num) >= recductionData.expression) && (recductionData.expression != 0)">满立减 : -{{recductionData.expression}}元</p>
        <p v-if="recductionData.expression == 0"></p>
      </div>
      <div class="pay">
        ...
        <!-- <p class="yingfu r">应付金额：
            <span v-if="deliveryMoney == -1">
              {{((total * (Number(discountData.discount)/100)) - Number(couponPrice)  - recductionData.expression)|keep2Num}} 元</span>
            <span v-else-if="deliveryMoney">
              {{((total * (Number(discountData.discount)/100)) + Number(deliveryMoney) - Number(couponPrice) - recductionData.expression)|keep2Num}} 元
            </span>
            <span v-else>
              {{((total * (Number(discountData.discount)/100)) + freight_price - Number(couponPrice) - recductionData.expression)|keep2Num}} 元</span>
        </p> -->
        <p class="yingfu r">应付金额：
            <span v-if="deliveryMoney == -1">
              {{((total * (Number(discountData.discount)/100)) - Number(couponPrice))|keep2Num}} 元</span>
            <span v-else-if="deliveryMoney">
              {{((total * (Number(discountData.discount)/100)) + Number(deliveryMoney) - Number(couponPrice))|keep2Num}} 元
            </span>
            <span v-else>
              {{((total * (Number(discountData.discount)/100)) + freight_price - Number(couponPrice))|keep2Num}} 元</span>
        </p>
        <div class="r">
...
<script>
  ...
  export default {
    ...
    head: {
        ...
        link: function(){
          return [
            { rel: 'shortcut icon', href: 'http://center.shopsn.cn'+this.headParams.link, id:'pcLink'},
          ]  
        }
    },
    filters:{
    ...
    methods: {
      ...
      placeOrder() {
        ...
        if (Object.keys(this.invoiceGroup).length === 0) {
          console.log(111111111, this.storeData.id)
        ...
        if (this.deliveryMoney == -1) {
          // this.totalmoney = this.total -Number((this.total|this.keep2Num)*(1-(Number(this.discountData.discount)/100)).toFixed(2))-this.totalReduction;
          // this.totalmoney = (this.total * (Number(this.discountData.discount)/100)) - Number(this.couponPrice)  - this.recductionData.expression
          this.totalmoney = (this.total * (Number(this.discountData.discount)/100)) - Number(this.couponPrice)
        }
        else if (this.deliveryMoney) {
          // this.totalmoney = this.total + Number(this.deliveryMoney)-Number((this.total|this.keep2Num)*(1-(Number(this.discountData.discount)/100)).toFixed(2))-this.totalReduction;
          // this.totalmoney = (this.total * (Number(this.discountData.discount)/100)) + Number(this.deliveryMoney) - Number(this.couponPrice) - this.recductionData.expression
          this.totalmoney = (this.total * (Number(this.discountData.discount)/100)) + Number(this.deliveryMoney) - Number(this.couponPrice)
        }
        else {
          // this.totalmoney = this.total + this.freight_price-Number((this.total|this.keep2Num)*(1-(Number(this.discountData.discount)/100)).toFixed(2))-this.totalReduction;
          // this.totalmoney = (this.total * (Number(this.discountData.discount)/100)) + this.freight_price - Number(this.couponPrice) - this.recductionData.expression
          this.totalmoney = (this.total * (Number(this.discountData.discount)/100)) + this.freight_price - Number(this.couponPrice)
        }
        var data = {
          panic_id: this.goodsData.panic_id,
          goods_id: this.goodsData.id,
          give_id: this.giftId,
          invoice_id: this.invoiceGroup,
          address_id: this.addressInfo.id,
          remarks: this.remarks,
          cut: this.recductionData.expression
        }
        ...
        this.HTTP(this.$httpConfig.getOrderBeginFromPanicGood, data, 'post').then((res) => {
          this.$router.push({
            path: '/pay',
            query: {
      ...
      //订单数据
      getOrderData() {
        this.HTTP(this.$httpConfig.getSeckillBuyDetail, {
        ...
        //   let temp = res.data.data.reduction
        //   // let temp = []
        //   if(Array.isArray(temp)){
        //       this.recductionData = {expression: '0', full: '0', name: ''}
        //   } else {
        //       this.recductionData = res.data.data.reduction;
        //   }
        //   this.totalReduction = res.data.data.reduction.expression;
        ...
        //   this.goodsreduce = res.data.data.reduction;
          ...
          this.total = parseFloat(this.goodsData.activity_price) * parseFloat(this.goodsData.goods_num);
        })
      },
      toggle(id) {
        this.store_id = id;
