                      ...
                      <input type="radio" name="tabset" id="tab1" aria-controls="all" checked>
                        ...
                        <div class="tab-panels ">
                            <section id="all" class="tab-panel left">
                              ...
                              <div v-for="item in items.goods_info" :key="item.id" class="details" >
                                <img  :src="URL+item.pic_url"  alt="">
                                  <div class="goodsinfo"  >
                                    <div class="title">{{items.title}}  <span>店铺推荐</span></div>
                            ...
                            <section id="promo" class="tab-panel left" >
                              <div v-for="(items,index) in promo" :key="index" class="new" style ="padding: 10px 0px 25px 0px;" v-if="items.goods_id">
                            ...
                            <section id="shopNews" class="tab-panel left">
                              <div class="shop_aeticle">  
                                <div  v-for="(items,index) in articlelist" :key="index">
                                <p>文章分类 : 
                                  <span  @click="all(items.name)" class="all">全部</span>
                                  <span class="article_types">{{items.name}}</span> 
                                </p>
                                </div>
                                <div v-for="(items,index) in articlelist" :key="index" class="article">
                                    <h4>{{items.title}}</h4>
...
<script>
  ...
  export default {
    ...
    created(){  
        let title = this.$route.query.class_name + '-' + sessionStorage.getItem('titleKey') + '-' +sessionStorage.getItem('updateDescription');
    ...
    methods:{
      ...
      promotion(){
        this.HTTP(this.$httpConfig.getPromoProductByStore,{store_id : 2},"post").then(res => {
              this.promo = res.data.data.PromotionGoods;
            })
            .catch(e => {
                console.log(e);
            });
      ...
      article(){
        this.HTTP(this.$httpConfig.getStoreArticle,{store_id : 2},"post").then(res => {
