		...
		<div class="center">
			...
			<div class="bottom">
				<div class="left l">
					...
					<ul>
						<li>订单编号：{{info.order_id}}</li>
						<li>退款编号：{{info.id}}</li>
						<li v-if="info.create_time">申请时间：{{info.create_time}}</li>
				...
				<div class="right r">
					...
					<div class="shangpin"><img :src="URL+info.pic_url" />
					...
					<ul class="ul">
						...
						<li>成交时间：{{info.pay_time | formatDate}}</li>
					</ul>
				</div>
			</div>
		</div>
    ...
    getFootData() {
      this.HTTP(this.$httpConfig.aboutEtcetera, {}, "post")
        .then(res => {
          ...
          let title='退款退货'+'-'+'查看详情'+'-'+ sessionStorage.getItem('titleKey') + '-' +sessionStorage.getItem('updateDescription');
