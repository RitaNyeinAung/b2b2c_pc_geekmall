<template>
	<div class="personal">
		<ul class="topul">
			<li class="l" @click="fun(0)" :class="{bg:isbg==0}"><span>基本信息</span></li>
			<li class="l" @click="fun(1)" :class="{bg:isbg==1}"><span>实名认证</span></li>
			<li class="l" @click="fun(2)" :class="{bg:isbg==2}"><span>头像信息</span></li>
		</ul>
					<!--基本信息-->
					<div class="xinxi" v-show="hidden==0">
						...
						<p class="nicheng"><span>*</span>昵称：<input type="text" v-model="userInfo.nick_name"/>
							<el-alert title="昵称不能为空" type="error" v-show="nameShow" show-icon :closable="false"></el-alert><br />
							<span>与平台业务或者店铺名称冲突的昵称，平台将有可能收回</span>
						</p>
					...
					<div class="renzheng" v-show="hidden==1">
						...
						<div class="approvalStatus" v-if="userInfo.authentication==2">
							<img src="../../../assets/img/approve.png"/>
							<span>恭喜您，已通过身证认证！</span>
						</div>
						<div v-else>
							<p class="name"><span class="span">*</span>真实姓名：
							<input type="text" v-model="userInfo.person_name" v-if="userInfo.authentication==3" :disabled="true"/>
							<input type="text" v-model="userInfo.person_name" v-else/>
								<el-alert title="真实姓名不能为空" type="error" v-show="realNameShow" show-icon :closable="false"></el-alert><br />
								<span class="l">身份证号码位数为15位或18位</span>
							</p>
							<p class="haoma"><span class="span">*</span>身份证号码：
							<input type="text" v-model="userInfo.id_card" v-if="userInfo.authentication==3" :disabled="true"/>
							<input type="text" v-model="userInfo.id_card" v-else/>
								<el-alert title="身份证号码不能为空" type="error" v-show="idNumberShow" show-icon :closable="false"></el-alert>
							</p>
							<div class="zhengmian both" v-if="userInfo.authentication==3">
								...
								<p class="l">实例：</p>
								<p class="l photos same">
									<img v-if="userInfo.card_positive==null" src="../../../assets/img/photo.png"/>
									<img v-else :src="URL + userInfo.card_positive" alt="">	
								</p>
								<el-alert title="实例不能为空" type="error" v-show="imGfrontSucceedShow" show-icon :closable="false"></el-alert>
							</div>
							<div class="zhengmian both" v-else>
								<p class="l"><span class="span">*</span>身份证正面照:</p>
								<p class="l photo same">
									<el-upload
										class="upload-demo"
							...
							<div class="beimian both" v-if="userInfo.authentication==3">
								...
								<p class="l">实例：</p>
								<p class="l photos same">
									<img  v-if="userInfo.card_side==null" src="../../../assets/img/photo.png"/>
									<img v-else :src="URL + userInfo.card_side" alt="">
								</p>
								<el-alert title="身份证背面照不能为空" type="error" v-show="imGcontrarySucceedShow" show-icon :closable="false"></el-alert>
							</div>
							<div class="beimian both" v-else>
								<p class="l"><span class="span">*</span>身份证背面照:</p>
								<p class="l photo same">
									<el-upload
										class="upload-demo"
							...
							<div class="jianyi"> 图片建议使用4：3比例，尺寸建议为1200*900像素jpg、gif、png格式的图片，并且图片大小不可超过2M。</div>
							<button @click="identityInformation" v-if="userInfo.authentication==null">保存</button>
							<button @click="identityInformation" v-if="userInfo.authentication==1">审核中</button>
							<button v-if="userInfo.authentication==3">保存</button>
						</div>
					</div>
					<!--头像信息-->
...
<script>
  ...
  data () {
    return {
	  userInfo:{},
		...
		realNameShow:false,
		idNumberShow:false,
		imGcontrarySucceedShow:false,
		imGfrontSucceedShow:false,
		successShow:false,
		pjtid:'',
  ...
  created () {
		...
		// }
	 	this.pjtid = this.$route.query.t;
	  	this.tabex=this.$route.query.x;
		this.hidden=this.tabex;
		this.isbg=this.tabex;
	  window.onbeforeunload = () => {
		sessionStorage.removeItem('successShow');
	  }
	  this.getInfo();
  ...
  methods: {
			...
			this.HTTP(this.$httpConfig.aboutEtcetera, {}, "post")
				.then(res => {
					...
					sessionStorage.setItem("contentKey", res.data.data.init_key_word);
						let title='个人资料'+'-'+ sessionStorage.getItem('titleKey') + '-' +sessionStorage.getItem('updateDescription');
						this.showScroll.scrollTitle(title);
				})
				.catch(err => {
					console.log(err);
		...
		// },
		identityInformation(){
			if(!this.userInfo.person_name){
			  this.realNameShow = true;
			  return;
			}

			if(!this.userInfo.id_card){
			  this.idNumberShow = true;
			  return;
			}
			if(!this.userInfo.card_positive){
			  this.imGfrontSucceedShow = true;
			  return;
			}
			if(!this.userInfo.card_side){
			  this.imGcontrarySucceedShow = true;
			  return;
			}
			this.HTTP(this.$httpConfig.getidentityAuthentication,{
				card_positive:this.userInfo.card_positive,
				card_side:this.userInfo.card_side,
		...
		contrarySucceed(response, file, fileList){
			this.userInfo.card_side = response.data;
		},
		// 正面上传成功
		frontSucceed(response, file, fileList){
			this.userInfo.card_positive = response.data;
		},
		//正面路径
		front(){      
	  	// this.HTTP(this.$httpConfig.getuploadIdCard,{},'post')
		// 	.then((res) =>{
	  ...
	  getInfo(){
	  	...
	  	// getUserAuth(){
		//     this.HTTP(this.$httpConfig.userAuth,{},'post')
		// 	.then((res) =>{
		// 		this.userAuthentication = res.data.data;
		// 	})
	  	// },
	  handleDate(date){
...
<style lang="less" scoped>
	...
	.personal{
			...
			.renzheng{
				...
				input{width:286px;height:30px;border:1px solid #ccc;}.span{color:#ff5454;margin-right:11px;}
				.name{margin: 30px 0 0 65px;input{margin-left: 17px;background-color: #fff;}span:nth-of-type(2){margin: 8px 0 0 92px;font-size:12px;color:#999;}
				.el-alert{width: 200px;padding: 6px 16px;float: right;margin-right: 55px;}
				}
				.haoma{margin: 36px 0 0 55px;input{margin-left:17px;background-color: #fff;}
				.el-alert{width: 200px;padding: 6px 16px;float: right;margin-right: 55px;}
				}
				.both{
				...
				.zhengmian{p:nth-of-type(1){margin-top: 33px;}p:nth-of-type(3){margin-top: 33px;}.el-alert{width: 150px;padding: 6px 16px;float: right;margin-right: 10px;margin-top: 35px;}}
				.beimian{p:nth-of-type(1){margin-top: 73px;}p:nth-of-type(3){margin-top: 73px;}.el-alert{width: 150px;padding: 6px 16px;float: right;margin-right: 10px;margin-top: 35px;}}
