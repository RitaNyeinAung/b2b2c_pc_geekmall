<template>
	<div class="account">
		<div class="logo">
			<img @click="toHome" :src="URL + logoPhoto" />
			<p>结算页</p>
		</div>
		<div class="center">
			<ul class="buzhou">
					...
					<div class="dianpu" v-for="(item, index) in buyData.store" :key="index">
						...
						<ul class="order-list">
							<li class="order-info" v-if="buyData.shop_goods_info[item.id]" v-for="(value, key) in buyData.shop_goods_info[item.id]" :key="key">
								...
								<span style="color: #dd2727;" v-if="value.price_member * value.goods_num>=value.reduction.expense">满￥{{value.reduction.expense}}，立减￥{{value.reduction.cut}}</span>
								
								<p v-if="deliveryMoney == -1" class="r">免配送费</p>
                				<p v-else-if="deliveryMoney" class="r">	
								<span v-if="value.price_member*value.goods_num>=value.reduction.expense">{{value.price_member*value.goods_num - value.reduction.cut + deliveryMoney|keep2Num}}</span>
								<span v-else>{{value.price_member*value.goods_num + deliveryMoney|keep2Num}}</span>
								<br>(含配送费：{{deliveryMoney|keep2Num}})</p>
								<p v-else-if="freight_price == 0" class="r">卖家包邮</p>
                				<p v-else class="r">
								<span v-if="value.price_member*value.goods_num>=value.reduction.expense">{{value.price_member*value.goods_num - value.reduction.cut + freight_price|keep2Num}}</span>
								<span v-else>{{value.price_member*value.goods_num + freight_price|keep2Num}}</span>	
								<br>(含配送费：{{freight_price|keep2Num}})</p>
							</li>
							
							<li class="order-info gift-info" v-if="giftData.length != 0" v-for="(gift, index) in giftData" :key="index">
								<img @click="toDetailsItem(gift.id)" class="l" :src="URL + gift.pic_url" alt=""/>
								<p @click="toDetailsItem(gift.id)" class="l">{{gift.title}}</p>
								<p class="l"></p>
								<p class="l">{{gift.price_member}}</p>
								<p class="l">{{gift.give_num}}</p>
								<p class="l" style="color: #dd2727;"></p>
								<p class="r"></p>
							</li>
							...
							<li>
								...
								<el-tabs v-show="cheap" v-model="activeName2[index]" type="card" @tab-click="handleClick(index)">
									...
									<el-tab-pane label="不可用" name="second">
										<ul class="voucherBox">
											<li v-if="item.id === voucher.store_id" v-for="(voucher, index) in vouchers.do_not_use"  :key="index" @click="checkCoupon(voucher.c_id, voucher.id, voucher.money)">
												<div class="MoneyBox">
													<span class="money">{{'￥' + voucher.money}}</span>
												</div>
												<div class="col">
...
<!--							<p>优惠券：-￥{{totalCoupon}}</p>-->
				...
				<p v-if="totalReduction">满立减：-{{totalReduction}} 元</p>
				<p v-else>满立减：-0 元</p>
			</div>
			<div class="pay">
<!--							<p class="yingfu r">应付金额：<span>￥{{((total_price + freight_price - totalCoupon + Number(deliveryMoney))|keep2Num)-Number((total_price|keep2Num)*(1-(Number(discountData.discount)/100)).toFixed(2))}}</span></p>-->
				<p class="yingfu r">应付金额：
					...
					<span v-if="deliveryMoney == -1">{{((total_price - totalCoupon)-Number((total_price)*(1-(Number(discountData.discount)/100)).toFixed(2))-(totalReduction))|keep2Num}} 元</span>
					<span v-else-if="deliveryMoney">{{((total_price - totalCoupon + 2*Number(deliveryMoney))-Number((total_price)*(1-(Number(discountData.discount)/100)).toFixed(2))-(totalReduction))|keep2Num}} 元</span>	
					<span v-else>{{((total_price - totalCoupon + 2*freight_price)-Number((total_price)*(1-(Number(discountData.discount)/100)).toFixed(2))-(totalReduction))|keep2Num}} 元</span>
					</span>
					<span v-else>
					<span v-if="deliveryMoney == -1">{{((total_price - totalCoupon)-Number((total_price)*(1-(Number(discountData.discount)/100)).toFixed(2))-(totalReduction))|keep2Num}} 元</span>
					<span v-else-if="deliveryMoney">{{((total_price + Number(deliveryMoney) - totalCoupon)-Number((total_price)*(1-(Number(discountData.discount)/100)).toFixed(2))-(totalReduction))|keep2Num}} 元</span>	
					<span v-else>{{((total_price + freight_price - totalCoupon)-Number((total_price)*(1-(Number(discountData.discount)/100)).toFixed(2))-(totalReduction))|keep2Num}} 元</span>
					</span>
				</p>
				<div class="r">
		...
		methods: {
			...
			placeOrder() {
				...
				if(Object.keys(this.params_goods).length === 0){
					for (let index = 0; index < this.store.length; index++) {
						this.params_goods[this.store[index].id] = '';
					}
				}
				if(this.store.length > 1) {
					if(this.deliveryMoney == -1) {
						var total = parseFloat(this.buyData.total_price)-Number((this.total_price|this.keep2Num)*(1-(Number(this.discountData.discount)/100)).toFixed(2))-(this.totalReduction)
					}
					else if(this.deliveryMoney) {
						var total = parseFloat(this.buyData.total_price) + 2*Number(this.deliveryMoney)-Number((this.total_price|this.keep2Num)*(1-(Number(this.discountData.discount)/100)).toFixed(2))-(this.totalReduction)
					}
					else {
						var total = parseFloat(this.buyData.total_price) + 2*Number(this.freight_price)-Number((this.total_price|this.keep2Num)*(1-(Number(this.discountData.discount)/100)).toFixed(2))-(this.totalReduction)
					}
				}
				else {
					if(this.deliveryMoney == -1) {
						...
						var total = parseFloat(this.buyData.total_price)+parseFloat(this.deliveryMoney)-Number((this.total_price|this.keep2Num)*(1-(Number(this.discountData.discount)/100)).toFixed(2))-(this.totalReduction)
					}
					else {
						var total = parseFloat(this.buyData.total_price)+parseFloat(this.freight_price)-Number((this.total_price|this.keep2Num)*(1-(Number(this.discountData.discount)/100)).toFixed(2))-(this.totalReduction)
					}
				}
			...
			getcartOrder(){
				this.HTTP(this.$httpConfig.BuyCartGoods, {
					...
					this.totalReduction = res.data.data.reduction;
					for (const key in this.buyData.total_number) {
						this.total_num+=this.buyData.total_number[key];
					}
					let temp = res.data.data.shop_goods_info
					for(var propKey in temp) {
						if(temp.hasOwnProperty(propKey)) {
							var propValue = temp[propKey];
							this.giftData = propValue[1].give
						}
					}
				}).catch((res) => {
			...
			getDistributionFee(){ //获取配送端
				this.HTTP(this.$httpConfig.deliveryMoney,{store_id: this.store[0].id,address_id: this.addressInfo.id},'post').then((res)=>{
