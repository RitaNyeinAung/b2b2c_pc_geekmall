...
<div class="buycar" ref="buycart">
    ...
    <div class="center" v-if="cartShow">
        ...
        <div class="dianpu" v-for="(store, index) in cartGoodsList" :key="index">
          ...
          <div class="bottom">
            ...
            >
              ...
              <p class="l" ref="money">￥{{ (item.price_member * item.goods_num) | keep2Num }}</p>
              <div class="r yiru">
      ...
      <div class="cargo" v-show="Object.keys(delGoodsInfo).length != 0">
        ...
        <p class="l">
          <span>￥{{ delGoodsInfo.price_member}}</span>
  ...
  methods: {
    ...
    getCartList() {
      this.HTTP(this.$httpConfig.cartGoodsList, {}, "post").then(res => {
        if (res.data.status === 1) {
          this.gitGoods = res.data.data.goods;
          this.cartShow = true;
          this.cartMeg = false;
          this.cartGoodsList = [];
    ...
    allPriceNum() {
      ...
      for (var j = 0; j < this.goods.length; j++) {
        ...
        this.selectedPrice +=
          parseFloat(this.goods[j].price_member) * this.goods[j].goods_num;
      }
    },
    // 店铺全选
    allShop(index) {
